...
function processTrigger(msg, cfg, snapshot = {}) {
  ...
  async function fetchAll(options) {
    try {
      const result = [];
      const articlesResponse = await request.get(options);
      const articlesObj = JSON.parse(articlesResponse);
  ...
  async function getArticles() {
    try {
      ...
      const options = {
        uri: `${cfg.url}/plugin/wp_wice_client_api_backend/json?method=get_all_articles&cookie=${cookie}`,
        headers: { 'X-API-KEY': cfg.apikey },
      };

      articles = await fetchAll(options);
      if (!articles || !Array.isArray(articles)) return `Expected records array. Instead received: ${JSON.stringify(articles)}`;
