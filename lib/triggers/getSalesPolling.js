...
const { saleToOih } = require('../transformations/saleToOih');
const { getSales } = require('./../utils/helpers');
...
async function processTrigger(msg, cfg, snapshot = {}) {
  try {
    ...
    const chances = await getSales(cfg, snapshot);

    const oihMeta = {};

    console.log(`Found ${chances.length} new sales.`);
    ...
    if (chances.length > 0) {
      chances.forEach((elem) => {
        ...
        const transformedElement = transform(newElement, cfg, saleToOih);

        if (cfg.devMode) console.log('Sale', transformedElement);
