...
function processTrigger(msg, cfg, snapshot = {}) {
  ...
  async function getCategoryRowid(cookie) {
    try {
      const uri = `${cfg.url}/plugin/wp_wice_client_api_backend/json?method=get_all_companies&cookie=${cookie}`;
  ...
  async function fetchAll(options) {
    try {
      const result = [];
      const organizationsResponse = await request.get(options);
      const organizationsObj = JSON.parse(organizationsResponse);
  ...
  async function getDeletedOrganizations() {
    try {
      ...
      const uri = `https://oihwice.wice-net.de/plugin/wp_wice_client_api_backend/json?method=get_all_companies&full_list=1&address_category1=${categoryId}&select_category1=${categoryId}&ext_search_do=1&cookie=${cookie}`; // eslint-disable-line
      const options = {
        uri,
        headers: {
      ...
      if (!organizations || !Array.isArray(organizations)) return `Expected records array. Instead received: ${JSON.stringify(organizations.loop_addresses)}`; // eslint-disable-line
