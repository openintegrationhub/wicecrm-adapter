...
async function processAction(msg, cfg) {
  try {
    ...
    if ('name' in msg.data) {
      isPerson = false;
      transformedMsg = transform(msg, cfg, organizationFromOih);
    } else {
      transformedMsg = transform(msg, cfg, personFromOih);
    }
   ...
   */
    ...
    let reply;
    if (isPerson) {
      reply = await upsertObject(transformedMsg, cfg, 'person');
    } else {
      reply = await upsertObject(transformedMsg, cfg, 'organization');
    }
