...
function processTrigger(msg, cfg) {
  ...
  async function getPerson(options) {
    try {
      ...
      if (!personObj.rowid) return `No person with ROWID: ${msg.body.rowid} found...`;
  ...
  async function executeRequest() {
    try {
      ...
      const options = {
        uri: `${cfg.url}/plugin/wp_wice_client_api_backend/json?method=get_person&cookie=${cookie}&pkey=${msg.body.rowid}`,
        headers: {
