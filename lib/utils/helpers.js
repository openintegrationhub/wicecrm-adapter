...
async function getChances(cfg, snapshot) {
  try {
    ...
    if (result && result.loop_entries && result.loop_entries.length > 0) {
      console.log(result.loop_entries.length, 'chances found');
      for (let i = 0; i < result.loop_entries; i += 1) {
        if (Date.parse(result.loop_entries[i].last_update) < cfg.lastUpdated) {
...
async function createArticle(data, cfg) {
  try {
    ...
    const options = {
      method: 'POST',
      uri: `${addProtocol(cfg.url)}/pserv/base/json_backend/article/save_article`,
      headers: { cookie: `wice_cookie=${cookie}` },
      json: true,
      simple: false,
      data,
...
module.exports = {
  createSession, getPersons, getOrganizations, upsertObject, getTickets, addNoteToTicket, createTicket, createArticle, getChances,
};
