...
async function checkForExistingPersonV2(person, options) {
  ...
  try {
    if (person.metadata.recordUid) {
      ...
      newOptions.qs = { method: 'get_contact', rowid: person.metadata.recordUid };

      const rowidObj = await request(newOptions);

      if (rowidObj && rowidObj.rowid) {
        existingRowid = rowidObj.rowid;
        console.log(`Person already exists ... ROWID: ${existingRowid}`);
      }
    }
    return existingRowid;
...
async function createOrUpdatePersonV2(existingRowid, options, msg) {
  try {
    ...
    newOptions.body = msg.data;
    newOptions.method = 'POST';

    if (!existingRowid || existingRowid === 0) {
      console.log('Creating person ...');
      newOptions.qs = { method: 'insert_contact' };
      const person = await request(newOptions);
      return person;
    }

    console.log('Updating person ...');
    newOptions.body.rowid = existingRowid;
    newOptions.qs = { method: 'update_contact' };
...
async function checkForExistingOrganizationV2(organization, options) {
  ...
  try {
    if (organization.metadata.recordUid) {
      const newOptions = options;
      newOptions.method = 'GET';
      newOptions.qs = { method: 'get_company', rowid: organization.metadata.recordUid };
...
async function createOrUpdateOrganizationV2(existingRowid, options, msg) {
  try {
    const newOptions = options;
    ...
    if (!existingRowid || existingRowid === 0) {
      newOptions.qs = { method: 'insert_company' };
      const organization = await request(newOptions);
      return organization;
    }

    console.log('Updating organization ...');
    newOptions.body.rowid = existingRowid;
    newOptions.qs = { method: 'update_company' };
...
async function upsertObjectV2(msg, cfg, type) {
  try {
    ...
    const options = {
      uri: `${addProtocol(cfg.url)}/plugin/wp_wice_client_api_backend/json?cookie=${cookie}`,
    };
