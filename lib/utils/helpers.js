...
async function createSession(cfg) {
  ...
  const options = {
    ...
    headers: { 'X-API-KEY': (cfg.key) ? cfg.key : cfg.apikey },
  };
...
async function getTickets(cfg, snapshot) {
  try {
    ...
    const { loop_tickets } = mainview;

    const promises = [];

    for (let i = 0; i < loop_tickets.length; i += 1) {
      promises.push(
        request({
          method: 'get',
          uri: `${addProtocol(cfg.url)}/pserv/base/json_backend/ticket/detailview/${loop_tickets[i].ticket_id}`,
          headers: { cookie: `wice_cookie=${cookie}` },
          json: true,
          simple: false,
        }),
...
module.exports = {
  createSession, getPersons, getOrganizations, upsertObject, getTickets, addNoteToTicket,
};
