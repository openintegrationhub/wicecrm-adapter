...
async function checkForExistingPerson(person, cookie, options) {
  let existingRowid = 0;
  try {
    if (person.meta.recordUid) {
      const newOptions = options;
      newOptions.form = {
        method: 'get_person',
        cookie,
        pkey: person.meta.recordUid,
      };
...
async function checkForExistingOrganization(organization, cookie, options) {
  let existingRowid = 0;
  try {
    if (organization.meta.recordUid) {
      const newOptions = options;
      newOptions.form = {
        method: 'get_company',
        cookie,
        pkey: organization.meta.recordUid,
      };
...
async function upsertObject(msg, cfg, type) {
  try {
    ...
    const options = {
      method: 'POST',
      uri: `${cfg.url}/plugin/wp_wice_client_api_backend/json`,
      headers: {
    ...
    if (type === 'person') {
      ...
      result = createOrUpdateOrganization(existingRowid, cookie, options, msg);
    }
    return result;
  } catch (e) {
    throw new Error(e);
